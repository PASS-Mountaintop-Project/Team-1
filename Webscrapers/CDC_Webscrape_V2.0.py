...
base_file_path = "C:/Mountaintop/Raw_Data/API_PLACES_Files" #Change if necessary
...
def getData(ID, year, format):
    ...
    df.to_csv(f"{base_file_path}/Raw_PLACES_{format}_CASTHMA_{year}.csv", index=False)

    df = pd.DataFrame.from_records(results_riskbeh)
    df.to_csv(f"{base_file_path}/Raw_PLACES_{format}_RISKBEH_{year}.csv", index=False)

    df = pd.DataFrame.from_records(results_prevent)
    df.to_csv(f"{base_file_path}/Raw_PLACES_{format}_PREVENT_{year}.csv", index=False)



for year in years:
    name = base_name.replace("{YEAR}", str(year)) #str(year)
